#include "level.h"
#include "floor.h"
#include "wall.h"
#include "portal.h"
#include "character.h"
#include "switch.h"
#include "door.h"
#include"pit.h"
#include "ramp.h"
#include <typeinfo>
Level::Level(int height, int width,  const std::string& l) : height(height),
    width(width),l(l)
{
    int count = 0;
    Portal* portal1 = nullptr;
    Portal* portal2 = nullptr;
    bool portal1found = false;


    map.resize(height, std::vector<Tile*>(width, nullptr));

    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            if (l.at(count) == '#')
            {
                map.at(i).at(j) = new Wall("#", i, j);
            }
            else if (l.at(count) == '.')
            {
                map.at(i).at(j) = new Floor(".", i, j);
            }
            else if (l.at(count) == 'O')
            {
                if (portal1found)
                {
                    portal2 = new Portal("O", i, j);
                    map.at(i).at(j) = portal2;
                    portal2->setDestination(portal1);
                    portal1->setDestination(portal2);
                    portal1found = false;
                }
                else
                {
                    portal1 = new Portal("O", i, j);
                    map.at(i).at(j) = portal1;
                    portal1found = true;
                }
            }
            else if (l.at(count) == 'C')
            {
                map.at(i).at(j) = new Floor(".", i, j);
                Character* character = new Character("C");
                placeCharacter(character, i, j);
                character->setCurrentTile(map.at(i).at(j));
            }
            else if(l.at(count)== '?'){
                Switch * newswitch = new Switch("?",i,j);
                map.at(i).at(j) = newswitch;


            }
            else if(l.at(count)== 'X'){
                door* newdoor = new door("X",i,j);
                map.at(i).at(j) = newdoor;



            }
            else if(l.at(count)== '_'){
                map.at(i).at(j) = new pit("_",i,j);
            }
            else if(l.at(count)== '<'){
                map.at(i).at(j) = new ramp("<",i,j);
            }
            count++;
        }
    }
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            auto* switchtile = dynamic_cast<Switch*>(map.at(i).at(j));
            if(switchtile!= nullptr){
                for (size_t i = 0; i < height; i++)
                {
                    for (size_t j = 0; j < width; j++)
                    {
                        auto* doortile = dynamic_cast<door*>(map.at(i).at(j));
                        if(doortile!= nullptr){
                            switchtile->attach(doortile);
                        }
                    }
            }

        }
    }
}
}
Level::Level(const Level& level):height(level.height),width(
          level.width), l(level.l){
    Portal* portal1 = nullptr;
    Portal* portal2 = nullptr;
    bool portal1found = false;

     map.resize(height, std::vector<Tile*>(width, nullptr));
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            auto originaltile = level.map[i][j];
            if(typeid(*originaltile)== typeid(Wall)){
                map[i][j]= new Wall("#", i, j);
            }
            else if(typeid(*originaltile)== typeid(Floor)){
                map.at(i).at(j) = new Floor(".", i, j);
            }
            else if(typeid(*originaltile)== typeid(Portal)){
                if (portal1found)
                {
                    portal2 = new Portal("O", i, j);
                    map.at(i).at(j) = portal2;
                    portal2->setDestination(portal1);
                    portal1->setDestination(portal2);
                    portal1found = false;
                }
                else
                {
                    portal1 = new Portal("O", i, j);
                    map.at(i).at(j) = portal1;
                    portal1found = true;
                }
            }
            else if(typeid(*originaltile)== typeid(Switch)){
                map.at(i).at(j) = new Switch("?", i, j);
            }
            else if(typeid(*originaltile)== typeid(door)){
                map.at(i).at(j) = new door("X", i, j);
            }
            else if(typeid(*originaltile)== typeid(pit)){
                map.at(i).at(j) = new pit("_", i, j);
            }
            else if(typeid(*originaltile)== typeid(ramp)){
                map.at(i).at(j) = new ramp("<", i, j);
            }
        }
        }
    for(const auto& character : level.characters){
            Character* newcharacter = new Character(*character);
        characters.push_back(newcharacter);
            map.at(character->getCurrentTile()->getRow()).at(character->getCurrentTile()->getColumn())->setCharacter(
            newcharacter);
        newcharacter->setCurrentTile(map.at(character->getCurrentTile()->getRow()).at(character->getCurrentTile()->getColumn()));

    }

    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            auto* switchtile = dynamic_cast<Switch*>(map.at(i).at(j));
            if(switchtile!= nullptr){
                for (size_t i = 0; i < height; i++)
                {
                    for (size_t j = 0; j < width; j++)
                    {
                        auto* doortile = dynamic_cast<door*>(map.at(i).at(j));
                        if(doortile!= nullptr){
                            switchtile->attach(doortile);
                        }
                    }
                }

            }
        }
    }
}


Level& Level:: operator=(const Level& level){
    if(this== &level){
        return *this;
    }
    //free own resources first
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            delete map.at(i).at(j);
        }
    }
    map.clear();

    for (size_t i = 0; i < characters.size(); i++)
    {
        delete characters.at(i);
    }
    characters.clear();
    height = level.height;
    width = level.width;
    l= level.l;
    Portal* portal1 = nullptr;
    Portal* portal2 = nullptr;
    bool portal1found = false;

    map.resize(height, std::vector<Tile*>(width, nullptr));
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            auto originaltile = level.map[i][j];
            if(typeid(*originaltile)== typeid(Wall)){
                map[i][j]= new Wall("#", i, j);
            }
            else if(typeid(*originaltile)== typeid(Floor)){
                map.at(i).at(j) = new Floor(".", i, j);
            }
            else if(typeid(*originaltile)== typeid(Portal)){
                if (portal1found)
                {
                    portal2 = new Portal("O", i, j);
                    map.at(i).at(j) = portal2;
                    portal2->setDestination(portal1);
                    portal1->setDestination(portal2);
                    portal1found = false;
                }
                else
                {
                    portal1 = new Portal("O", i, j);
                    map.at(i).at(j) = portal1;
                    portal1found = true;
                }
            }
            else if(typeid(*originaltile)== typeid(Switch)){
                map.at(i).at(j) = new Switch("?", i, j);
            }
            else if(typeid(*originaltile)== typeid(door)){
                map.at(i).at(j) = new door("X", i, j);
            }
            else if(typeid(*originaltile)== typeid(pit)){
                map.at(i).at(j) = new pit("_", i, j);
            }
            else if(typeid(*originaltile)== typeid(ramp)){
                map.at(i).at(j) = new ramp("<", i, j);
            }
        }
    }
    for(const auto& character : level.characters){
        Character* newcharacter = new Character(*character);
        characters.push_back(newcharacter);
        map.at(character->getCurrentTile()->getRow()).at(character->getCurrentTile()->getColumn())->setCharacter(
            newcharacter);
        newcharacter->setCurrentTile(map.at(character->getCurrentTile()->getRow()).at(character->getCurrentTile()->getColumn()));

    }

    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            auto* switchtile = dynamic_cast<Switch*>(map.at(i).at(j));
            if(switchtile!= nullptr){
                for (size_t i = 0; i < height; i++)
                {
                    for (size_t j = 0; j < width; j++)
                    {
                        auto* doortile = dynamic_cast<door*>(map.at(i).at(j));
                        if(doortile!= nullptr){
                            switchtile->attach(doortile);
                        }
                    }
                }

            }
        }
    }
    return *this;
}

std::vector<Character*> Level::getCharacters() const
{
    return characters;
}

std::vector<std::vector<Tile*>> Level::getMap() const
{
    return map;
}

Level::~Level()
{
    for (size_t i = 0; i < height; i++)
    {
        for (size_t j = 0; j < width; j++)
        {
            delete map.at(i).at(j);
        }
    }
    map.clear();

    for (size_t i = 0; i < characters.size(); i++)
    {
        delete characters.at(i);
    }
    characters.clear();
}

Tile* Level::getTile(int row, int col)
{
    if (row >= 0 && row < map.size() && col >= 0 && col < map.at(row).size())
    {
        return map.at(row).at(col);
    }
    else
    {
        return nullptr;
    }
}

const Tile* Level::getTile(int row, int col) const
{
    if (row >= 0 && row < map.size() && col >= 0 && col < map.at(row).size())
    {
        return map.at(row).at(col);
    }
    else
    {
        return nullptr;
    }
}

void Level::placeCharacter(Character* c, int row, int col)
{
    if (row >= 0 && row < map.size() && col >= 0 && col < map.at(row).size())
    {
        Tile* targetTile = map.at(row).at(col);
        targetTile->setCharacter(c);
        characters.push_back(c);
    }
}

int Level::getHeight() const
{
    return height;
}

int Level::getWidth() const
{
    return width;
}



