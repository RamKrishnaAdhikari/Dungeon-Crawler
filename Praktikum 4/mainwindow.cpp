    ...
    dc(dc), cur_pos_row(0), cur_pos_col(0)
{
    ...
    level = dc->getLevel();
    tileTextures.resize(level->getHeight(), std::vector<int>(level->getWidth(), 0));
    ...
    for (int row = 0; row < level->getHeight(); row++)
    {
        for (int col = 0; col < level->getWidth(); col++)
        {
            ...
            if (*tex_c == '.')
            {
                tileTextures[row][col] = std::rand() % 5 + 1;  // Randomly select a floor texture index
                widget->setPixmap(QPixmap(":/textures/floor/floor" + QString::number(tileTextures[row][col]) + ".png"));
            }
            else if (*tex_c == 'C')
            {
                widget->setPixmap(QPixmap(":/textures/floor/floor1.png"));
...
void MainWindow::draw(Level* level)
{
    for (int row = 0; row < level->getHeight(); row++)
    {
        for (int col = 0; col < level->getWidth(); col++)
        {
            ...
            int tmp = rand() % 5; // Zufällige Zahl zwischen 0 und 4
            if (*tex_c == '.')
            {
            switch (tmp)
            {
...
void MainWindow::draw(Level* level)
{
    for (int row = 0; row < level->getHeight(); row++)
    {
        for (int col = 0; col < level->getWidth(); col++)
        {
            ...
            if (*tex_c == '.')
            {
                int textureIndex = tileTextures[row][col];
                widget->setPixmap(QPixmap(":/textures/floor/floor" + QString::number(textureIndex) + ".png"));
            }
            else if (*tex_c == '#')
            {
                widget->setPixmap(QPixmap(":/textures/wall/wall1.png"));
            ...
            {
                Portal* portal = dynamic_cast<Portal*>(level->getTile(row, col));
                int portalType = portal ? portal->getportaltype() : 1;  // Default to 1 if dynamic cast fails
                widget->setPixmap(QPixmap(":/textures/portal/portal" + QString::number(portalType) + ".png"));
            }
            else if (*tex_c == '?')
            {
